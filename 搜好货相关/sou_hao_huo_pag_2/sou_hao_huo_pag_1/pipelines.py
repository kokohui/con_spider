...
class SouHaoHuoPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        sql = 'insert into `bus_user` (' \
              '`name`, `logo`, `phone`, `password`, `source`, `type`, `state`, `plate_visit_num`, `plate_visit_pnum`,' \
              ...
              ' `ad_price`, `openid`, `unionid`, `provider_id`, `provider_name`, `channel_duty_id`, `channel_open_id`, `service_id`, `pk_user_main`, `keywords`) ' \
              'VALUE' \
        ...
        '1', item['create_date'], '0', item['list_img'], item['price'], item['title'], item['way'],
        item['one_level_id'], item['two_level_id'], item['three_level_id'], 0, item['keywords'], '',
        '', item['imgs'], '1', item['create_date'], '1', '1', 0,
        item['create_date'], '', item['detail'], '0', item['create_date'], item['create_date'], 1, item['units'],
        '元', '', '', '', '', '0', '0',
        '1ec40ecd3cf64908941b5f7679f19d2b', '', '1', item['com_name'], item['linkman'], item['mobile'],
