...
class SouHaoHuoPipeline(object):
        ...
        if result_count == 0:
            ...
            try:
                ...
                data = self.cur.execute(sql, (
                    ...
                    '', item['address'], '', item['summary'], '', item['scopes'], item['list_img'], '', '', '',
                    '', '', 123456, item['mobile'], '', 0, 0, 0, 0, 0,
                    0, '', '75cebe2e19434dcd9c4586f4621e6f9c', '', '', '', '', item['keywords'], 1))
            ...
            try:
                ...
                self.cur.execute(sql_in_2, (self.res_num, item['one_level_id'], item['two_level_id'], item['three_level_id'],'1','0'))
                self.conn.commit()  # 提交
                print('sql_in_2 添加成功')
            except Exception as e:
                raise e
    ...
    def close_spider(self, spider):
        sql_id = "SELECT id FROM bus_spider_data WHERE TYPE = 'chengxin' AND is_del = '0' AND isuse = '0' ORDER BY create_date LIMIT 1 "
