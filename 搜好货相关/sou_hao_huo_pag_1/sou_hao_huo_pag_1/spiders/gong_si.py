...
class GongSiSpider(scrapy.Spider):
    name = 'gong_si'
    url = 'http://s.912688.com/comp/dy/search?kw=%E8%BD%AE%E8%83%8E&pageSize=20&page=3'

    d_id_3 = '185'
    keywords_name = '大码女装'
    # start_url = 'http://b2b-1882196747.912688.com/product-1-1-315712815-1-2.html?spm=p201905.vshopproduct.catemenu.668ce86b-a57d-463e-809f-596ea4bc956c'
    num_2 = 1
    ...
    def parse(self, response):
        ...
        f_url_list = response.xpath('//div[@id="main"]/div[@class="floor-container overflow"]')
        for li_list in f_url_list:
            a_url_list =li_list.xpath('./div[1]/div[1]//li/a/@href').extract()
            for a_url in a_url_list:
                # print('a_url:', a_url)
                for num in range(2, 3):
                    page_url = a_url.split('.h')[0] + '-mod-page{}.html'.format(num)
                    yield Request(url=a_url, callback=self.parse_2)
...
def shuju(response, item, d_id_3_3, keywords_name_2, ):
    ...
    try:
        ...
        os.makedirs('d:\\深圳市多多乐家具有限公司\\{}'.format(str_ran))
        #     将图片链接保存到硬盘
        ...
        for img_url in res_img:
            ...
            with open('d:\\深圳市多多乐家具有限公司\\{}\\{}.jpg'.format(str_ran, img_name), 'wb') as f:
                f.write(code_img)
    ...
    com_name = '-'
    try:
        # com_name = response.xpath('/html/body/div[5]/div[5]/div[2]/ul/li[4]/span[3]/text()')[0].extract()
        com_name = response.xpath('//*[@id="head"]/div[@class="head-ff"]/div/div[@class="com-name"]/a/text()').extract()[0]
    ...
    scope_url = response.xpath('//*[@id="head"]/div[3]/div/ul/li[3]/a/@href').extract()[0]
    print(scope_url)
    # return item
    ...
    response_text = requests.get(url=scope_url, headers=headers).text
    tree = etree.HTML(response_text)
    # 公司简介
    summary = '-'
    try:
        summary = tree.xpath(
            '//*[@id="main"]/div[@class="wrap"]/div[@class="pb20 clearfix"]/div[@class="r-part R"]/div[@class="company-info section"]/div[@class="contain"]/p/text()')[0].strip()
    ...
    try:
        scopes = tree.xpath(
            '//*[@id="main"]/div[@class="wrap"]/div[@class="pb20 clearfix"]/div[@class="r-part R"]/div[@class="register-info section"]/div[@class="contain"]/table//td[@class="val val-max"]/text()')
        scopes = scopes[0].strip()
        print('scopes', scopes)
    except:
        print('scopes', scopes)
    item['scopes'] = scopes
    ...
    address = '-'
    try:
        address = re.findall('<li>.*?所在地区：</span><span.*?>(.*?)</span></li>', response.text, re.S)[0]
    except:
        print('没有')
    # str(res_phone)
